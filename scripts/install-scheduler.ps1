...
#
# Order: refresh-analytics → ingest-own-channel (if >24h stale) → score-principles → channel-benchmark
#
...
$Chain = @"
"$NodeExe" "$ScriptRoot\scripts\refresh-analytics.js" --age-filter 7 && "$NodeExe" "$ScriptRoot\scripts\ingest-own-channel.js" && "$NodeExe" "$ScriptRoot\scripts\score-principles.js" && "$NodeExe" "$ScriptRoot\scripts\channel-benchmark.js"
"@.Trim()
...
Register-ScheduledTask `
  ...
  -Settings  $Settings `
  -Principal $Principal `
  -Description "SleepForge nightly analytics: refresh → ingest → score → benchmark" | Out-Null
