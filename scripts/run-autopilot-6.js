 ...
 * run-autopilot-6.js
 *
 * 3-video fully autonomous autopilot — sleepless-philosophers
 * Schedule: Mon May 18 – Wed May 20 2026, 8am Bangkok (01:00 UTC)
 *
 * Pipeline per video:
 ...
 *   4. Generate 3 thumbnail variants (56px floor, hook validator, period-authentic, critic retry)
 ...
 *
 * Usage: node scripts/run-autopilot-6.js [--dry-run] [--keep-files]
 */
...
// ─── 3-VIDEO LINEUP ───────────────────────────────────────────────────────────
// Mon May 18 – Wed May 20 2026, 8am Bangkok = 01:00 UTC each day

const VIDEOS = [
  {
    index:        6,
    scheduledAt:  '2026-05-18T01:00:00Z',
    tradition:    'Platonic / Idealist',
    philosopher:  'Plato',
    philosophers: 'plato,aristotle',
    titlePattern: 'completeness_claim',
    draftTitle:   'All of Plato\'s Philosophy Explained | Sleep Meditation',
    topic: 'Platonic philosophy: the Theory of Forms, the Allegory of the Cave, the divided line of knowledge, the Republic and philosopher-kings, anamnesis (recollection of past lives), the tripartite soul, eros as ascent toward the Good, the Form of the Good as the highest reality',
  },
  {
    index:        7,
    scheduledAt:  '2026-05-19T01:00:00Z',
    tradition:    'Stoicism (Roman)',
    philosopher:  'Epictetus',
    philosophers: 'epictetus,marcus-aurelius',
    titlePattern: 'superlative_quality',
    draftTitle:   'The Most Calming Teachings of Epictetus for Deep Sleep',
    topic: 'Epictetus Stoic philosophy: the dichotomy of control, prohairesis (moral choice), the Enchiridion teachings, freedom through acceptance, suffering as misperception of events, the Stoic practice of daily philosophy, life as a banquet — take what is offered, the inner citadel no one can breach',
  },
  {
    index:        8,
    scheduledAt:  '2026-05-20T01:00:00Z',
    tradition:    'Stoicism (Roman)',
    philosopher:  'Seneca',
    philosophers: 'seneca,epicurus',
    titlePattern: 'duration_list',
    draftTitle:   '1 Hour of Seneca: Letters on Time and Mortality for Sleep',
    topic: 'Seneca\'s philosophy: Letters to Lucilius on the shortness of life, time as our most precious possession — we waste it on trivialities, tranquility of mind (De Tranquillitate Animi), anger as temporary madness, meditatio mortis (contemplation of death as preparation for living), the brevity of life, philosophy as daily practice and consolation',
  },
];
  ...
  return new Promise((resolve, reject) => {
    const child = spawn(process.execPath, [path.join(ROOT, 'scripts', 'test-video-2min.js')], {
      ...
      env: {
        ...
        FRAME_VARIANT:           String((video.index - 1) % 5),
      },
      cwd: ROOT,
    });
...
- Pattern meanings: encyclopedic_number = "30 Biggest X", completeness_claim = "All of X Explained", duration_list = "1 Hour of X", superlative_quality = "The Most Calming X"
...
function writeProgressReport(results, batchFile, startedAt) {
  ...
  const lines = [
    `# SleepForge — 3-Video Scheduled Batch (autopilot-6)`,
...
async function main() {
  ...
  log('\n╔══════════════════════════════════════════════════════╗');
  log('║   SleepForge — 3-Video Autonomous Batch (autopilot-6) ║');
  log('╚══════════════════════════════════════════════════════╝');
  log(`  Channel:  ${CHANNEL}`);
  ...
  let lastSuccessfulVideoPath = null;
  ...
  for (let i = 0; i < VIDEOS.length; i++) {
    ...
    logSection(`VIDEO ${i + 1} / 3 — ${video.tradition} (${video.philosopher})`);
    ...
    try {
      ...
      meta = { ...generated, title: video.title };
